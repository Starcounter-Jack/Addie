...
//
// Parts Copyright 2015 Marcus Zetterquist under the Apache license
...
template <class T>
    class vector : public Addie::Internals::Object {
...
bool vector<T>::check_invariant() const{
	...
	assert(_shift >= internals::EMPTY_TREE_SHIFT && _shift < 32);
	assert(_shift == internals::vector_size_to_shift(_size));
...
vector<T>::vector(internals::node_ref<T> root, std::size_t size, int shift) :
	...
	_shift(shift)
{
	assert(shift >= internals::EMPTY_TREE_SHIFT);
	assert(internals::vector_size_to_shift(size) == shift);
	assert(check_invariant());
}
...
const T& vector<T>::operator[](const std::size_t index) const{
	...
	assert(shift == internals::LEAF_NODE_SHIFT);
	assert(node_it->get_type() == internals::node_type::leaf_node);

	const auto slot_index = index & internals::BRANCHING_FACTOR_MASK;

	assert(slot_index < node_it->get_leaf_node()->_values.size());
